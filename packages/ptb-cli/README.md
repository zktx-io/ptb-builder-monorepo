...
Agents and scripts should use `--json` and parse stdout as one JSON document.
...
- Branch first on `ok`, then on `error.code` when `ok` is `false`.
- Treat `mermaid` as inspection output, not as signing, simulation, execution,
  or transaction safety input.
- Treat `diagnostics` as model inspection data. A successful command can still
  include diagnostics.
- Treat `error.cause` as optional debugging detail. SDK, network, and system
  messages are not stable user-facing strings.
...
| `input.unsupported` | Local input was not TransactionData hex, or an unsupported local input path such as TransactionKind, raw JSON, file, or stdin was used. |

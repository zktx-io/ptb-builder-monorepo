...
import { inferCastTarget, isTypeCompatible } from '../src/ptb/graph/typecheck';
...
describe('isTypeCompatible', () => {
  it.each([
    ...
    expect(isTypeCompatible(sourceType, targetType)).toBe(expected);
  });

  it('keeps missing object typeTag leniency at the top-level edge only', () => {
    expect(isTypeCompatible(object(), object(canonicalSui))).toBe(true);
    expect(
      isTypeCompatible(vector(object()), vector(object(canonicalSui))),
    ).toBe(false);
    expect(
      isTypeCompatible(option(object()), option(object(canonicalSui))),
    ).toBe(false);
    expect(
      isTypeCompatible(
        vector(object('0x2::sui::SUI')),
        vector(object(canonicalSui)),
      ),
    ).toBe(true);
    expect(
      isTypeCompatible(
        option(object('0x2::sui::SUI')),
        option(object(canonicalSui)),
      ),
    ).toBe(true);
  });
  ...
  it('rejects model-known non-object structs as object type tags', () => {
    [
      ...
      expect(isTypeCompatible(object(typeTag), object(typeTag))).toBe(false);
      expect(isTypeCompatible(object(), object(typeTag))).toBe(false);
      expect(isTypeCompatible(object(typeTag), object())).toBe(false);
      expect(
        isTypeCompatible(vector(object(typeTag)), vector(object(typeTag))),
      ).toBe(false);
      expect(
        isTypeCompatible(option(object(typeTag)), option(object(typeTag))),
      ).toBe(false);
    });
  });
